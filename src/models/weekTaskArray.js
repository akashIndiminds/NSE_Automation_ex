...
class weekTaskArray {
    static instance;
    constructor() {
      if (weekTaskArray.instance) {
        return weekTaskArray.instance;
      }
      this.logDate = this.updateLogDate();
      // console.log(this.logDate);
      this.array = [];
      this.filePath= path.resolve(`./src/WeekActivity/${this.logDate}.json`);
      this.loadTaskDataSync();
      weekTaskArray.instance = this;
    }
  ...
  updateLogDate() {
    ...
    const dateObj = getDateObj(previousMonday);
    const str = syncDateFormatReader(dateObj ,'DD-MM-YYYY');
    weekTaskArray.logDate = str;
     return str;
   }
    ...
    syncActivityLog(){
      ...
      if(weekTaskArray.instance.logDate != currentDate){
        console.log("📅 Detected date change! Rotating weeek activity log...");
        this.logDate = this.updateLogDate();
        console.log(this.logDate);
        this.array = [];
        this.filePath= path.resolve(`./src/WeekActivity/${this.logDate}.json`);
  ...
  const weekTaskInstance = new weekTaskArray();
  
  export default weekTaskInstance;
